...
class KNN:
    ...
    """
    def __init__(self, train_data=None, dimensions=None, axis=0, sel_axis=None):
        """
        ...
        self.kdtree = kdtree.create(self.train_data.keys(),dimensions,axis,sel_axis)
    ...
    def decision(self,neighbors=None):
        ...
        Returns an ordered list of (label, probability) tuples, key=probability.

        When neighbors is None, returns self.class_prb.
        """
        if not neighbors:
            return sorted(self.class_prb.items(), key=lambda n:n[1], reverse=True)
...
if __name__=="__main__":
    ...
    m.visualize_kdtree()
    f = lambda a, b: sum( abs(a[axis]-b[axis]) for axis in range(len(a))) # Manhattan distance
